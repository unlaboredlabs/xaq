...
printf 'xaq-edge-v1 %s\nxaq-linux-x86_64 %s %s\n' "$git_sha" "$asset" "$digest" > "$fixture/manifest"
request_log="$scratch/requests"

PATH="$mock_bin:$PATH" FIXTURE_DIR="$fixture" REQUEST_LOG="$request_log" \
    XAQ_INSTALL_DIR="$install_dir" sh "$repo/install.sh" >/dev/null
