...
class SelectionTests(unittest.TestCase):
    ...
    def test_typed_unicode_is_drawn_complete_and_edited_by_cluster(self):
        ...
        self.send(b"\xc3")
        self.drain()
        self.assertNotIn(b"\xc3", self.output[offset:])
        self.send(b"\xa9" + "界👩‍💻".encode())
        self.until(lambda: "é界👩‍💻".encode() in self.output[offset:])
        offset = len(self.output)
        self.send(b"\x7f")
        self.until(lambda: "é界".encode() in self.output[offset:])
        self.assertNotIn("👩‍💻".encode(), self.output[offset:])
        offset = len(self.output)
        self.send(b"\x1b[DX")
        self.until(lambda: "éX界".encode() in self.output[offset:])
        self.send(b"\x80\xc0\x80\xff")
        self.drain()
        self.output.decode("utf-8")

    def test_tiny_resize_clears_selection_and_restores_the_prompt(self):
        self.start(False)
        self.drag()
        self.until(lambda: b"\x1b[7m" in self.output)
        offset = len(self.output)
        fcntl.ioctl(self.master, termios.TIOCSWINSZ, struct.pack("HHHH", 1, 1, 0, 0))
        os.kill(self.process.pid, signal.SIGWINCH)
        self.until(lambda: b"\x1b[r\x1b[2J\x1b[H" in self.output[offset:])
        offset = len(self.output)
        fcntl.ioctl(self.master, termios.TIOCSWINSZ, struct.pack("HHHH", 24, 100, 0, 0))
        os.kill(self.process.pid, signal.SIGWINCH)
